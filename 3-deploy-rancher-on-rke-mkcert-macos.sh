...
# in /etc/hosts set
# 192.168.64.23 gui.rancher.svc rke1
# the ip 192.168.64.23 can be any ip of rke1, rke2 or rke3
export rancherip=`cat hosts | grep rke1 | awk '{print $1}'`
sudo -- sh -c "echo $rancherip gui.rancher.svc >> /etc/hosts"
...
helm install rancher rancher-stable/rancher --namespace cattle-system --set hostname=gui.rancher.svc  --set replicas=3 --set ingress.tls.source=secret --set privateCA=true
echo "############################################################################"
